...
def resnet18(pretrained=False,num_classes=10, include_top=True,model_root=None, **kwargs):
   model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, include_top=include_top, **kwargs)
   if pretrained:
       pretrain_dict = model_zoo.load_url(model_urls['resnet18'], model_root)
...
def resnet34(pretrained=False, num_classes=10, include_top=True,model_root=None, **kwargs):
    model = ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, include_top=include_top, **kwargs)
    if pretrained:
        pretrain_dict = model_zoo.load_url(model_urls['resnet34'], model_root)
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict)
...
def resnet101(pretrained=False, num_classes=10, include_top=True,model_root=None, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes=num_classes, include_top=include_top, **kwargs)
    if pretrained:
        pretrain_dict = model_zoo.load_url(model_urls['resnet34'], model_root)
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict}
