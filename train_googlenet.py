...
def data_tf(x):
    ...
    x =(x - 0.5) / 0.5
    # print(x.shape)  # 图片格式为96*96*3
    x = x.transpose((2, 0, 1))
    # print(x.shape)  # 转换成PyTorch支持的格式3*96*96
...
parser.add_argument('--epoch_', type=int, default=1, help='epoch to start training from')
...
misc.logger.init(args.logdir, 'log_2021_3_9_googlenet')
print = misc.logger.info
os.makedirs('googlenet_models3-9', exist_ok=True)
...
num_classes = 10
train_dataset=datasets.CIFAR10(root=r'./data1',train=True,transform=data_tf,download=True)
...
if args.epoch_ != 0:
    ...
    # the first parameter can set as the channel of image, the second parameter can set as the num_class
    model = googlenet.googlenet(3, 10)
...
for epoch in range(args.n_epochs):
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), 'googlenet_models3-9/model_%d.pth' % epoch)